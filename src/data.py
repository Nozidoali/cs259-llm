...
from config import TRUTHFULQA_CACHE_DIR, MODELS_DIR, MODEL_CONFIGS, DATASET_CONFIG
...
def download_model(model_key, output_dir=None):
    ...
    snapshot_download(repo_id=model_id, local_dir=str(output_dir), local_dir_use_symlinks=False, resume_download=True, max_workers=1)
...
def prepare_truthfulqa_dataset(tokenizer, max_length=512, keep_metadata=False, model_type="causal"):
    try:
        ds = hf_load_dataset(DATASET_CONFIG["name"], DATASET_CONFIG["config"], split=DATASET_CONFIG["split"], cache_dir=str(TRUTHFULQA_CACHE_DIR))
    except (TypeError, AttributeError, ValueError) as e:
        logger.warning(f"Cache error loading TruthfulQA: {e}, clearing cache and retrying...")
        cache_dirs = [str(TRUTHFULQA_CACHE_DIR), os.path.expanduser("~/.cache/huggingface/datasets/truthful_qa"), os.path.expanduser("~/.cache/huggingface/datasets/truthfulqa___truthful_qa")]
    ...
    def format_example(ex):
        ...
        if model_type == "seq2seq":
            ...
            text = DATASET_CONFIG["format_template"].format(question=question, best_answer=best_answer)
            result = {"text": text}
        if keep_metadata:
    ...
    if model_type == "seq2seq":
        def tokenize_seq2seq(examples):
            ...
            return inputs
        tokenized = formatted.map(tokenize_seq2seq, batched=True, remove_columns=["input", "target"] if not keep_metadata else ["input", "target"], desc="Tokenizing dataset")
    else:
        tokenized = formatted.map(lambda examples: tokenizer(examples["text"], truncation=True, max_length=max_length, padding="max_length"), batched=True, remove_columns=["text"] if not keep_metadata else [], desc="Tokenizing dataset")
    return tokenized
...
def prepare_qmsum_dataset(tokenizer, max_length=512, keep_metadata=False, model_type="causal", split=None, num_samples=None):
    if split is None:
        try:
            ...
            ds = hf_load_dataset("zai-org/LongBench", "qmsum", trust_remote_code=True)
            if isinstance(ds, dict):
                ds = ds.get("test") or ds.get("train") or list(ds.values())[0]
    else:
        ds = hf_load_dataset("zai-org/LongBench", "qmsum", split=split, trust_remote_code=True)
    ...
    if model_type == "seq2seq":
        def tokenize_seq2seq(examples):
            ...
            inputs["labels"] = targets["input_ids"]
            return inputs
        tokenized = formatted.map(tokenize_seq2seq, batched=True, remove_columns=["input", "target"] if not keep_metadata else ["input", "target"], desc="Tokenizing QMSum dataset (seq2seq)")
    else:
        tokenized = formatted.map(lambda examples: tokenizer(examples["text"], truncation=True, max_length=max_length, padding="max_length"), batched=True, remove_columns=["text"] if not keep_metadata else [], desc="Tokenizing QMSum dataset (causal)")
    return tokenized
