...
# Use pip-installed gguf package instead of local gguf-py
...
logger_conversion = logging.getLogger(__name__)
...
def convert_to_gguf(model_path: Path, output_file: Path, quantize_level: str = "f16") -> None:
    ...
    logger_conversion.info(f"Converting: {model_path}")
    logger_conversion.info(f"Output: {output_file}")
    logger_conversion.info(f"Quantization: {quantize_level}")
    ...
    logger_conversion.info(f"Loading model: {model_path.name}")
    
    with torch.inference_mode():
        output_type = ftype_map[quantize_level]
        model_type = ModelType.TEXT
        hparams = ModelBase.load_hparams(model_path, False)
        model_architecture = get_model_architecture(hparams, model_type)
        logger_conversion.info(f"Model architecture: {model_architecture}")
        try:
            model_class = ModelBase.from_model_architecture(model_architecture, model_type=model_type)
        except NotImplementedError:
            raise ValueError(f"Model {model_architecture} is not supported")
        
        model_instance = model_class(model_path, output_type, output_file,
                                     is_big_endian=False, use_temp_file=False,
                                     eager=False, metadata_override=None, model_name=None,
                                     split_max_tensors=0, split_max_size=0, dry_run=False,
                                     small_first_shard=False, remote_hf_model_id=None,
                                     disable_mistral_community_chat_template=False,
                                     sentence_transformers_dense_modules=False)
        
        logger_conversion.info("Exporting model...")
        model_instance.write()
        logger_conversion.info(f"Model successfully exported to {output_file}")
        logger_conversion.info(f"Complete: {output_file}")
...
def merge_experts_to_standard_mlp(model_path: Path, output_path: Path, merge_mode: str = "average"):
    ...
    if index_file.exists():
        logger_conversion.info("Loading from sharded safetensors format")
        ...
        logger_conversion.info("Loading from single safetensors file")
        with safe_open(str(model_path / "model.safetensors"), framework="pt", device="cpu") as f:
            ...
            logger_conversion.info(f"Loaded {len(keys)} tensors from model.safetensors")
    elif (model_path / "pytorch_model.bin.index.json").exists():
        logger_conversion.info("Loading from sharded pytorch format")
        ...
        logger_conversion.info("Loading from single pytorch file")
    ...
    logger_conversion.info(f"Total state dict keys: {len(state_dict)}")
    expert_indices = set()
    expert_keys = []
    ...
    logger_conversion.info(f"Found {num_experts} experts with indices: {sorted(expert_indices)}")
    logger_conversion.info(f"Total expert keys: {len(expert_keys)}")
    if expert_keys:
        logger_conversion.info(f"Sample expert keys (first 5): {expert_keys[:5]}")
    new_state_dict = {k: v for k, v in state_dict.items() if ".mlp.expert" not in k}
    logger_conversion.info(f"Base state dict (non-expert) keys: {len(new_state_dict)}")
    
    if num_experts > 0:
        logger_conversion.info(f"Merging {num_experts} experts using mode: {merge_mode}")
        merged_layers = 0
                ...
                for expert_idx in sorted(expert_indices):
                    ...
                    if expert_key in state_dict:
                        ...
                        if layer_idx == 0 and not layer_merged:
                            logger_conversion.info(f"Layer {layer_idx}: Found expert {expert_idx} for {proj_type}, shape: {state_dict[expert_key].shape}")
                if expert_weights:
                    ...
                    layer_merged = True
                    if layer_idx == 0:
                        logger_conversion.info(f"Layer {layer_idx}: Merged {len(expert_weights)} experts for {proj_type}, output shape: {merged_weight.shape}")
            if layer_merged:
                merged_layers += 1
        logger_conversion.info(f"Successfully merged experts in {merged_layers}/{config.num_hidden_layers} layers")
    else:
        logger_conversion.warning("No experts found in model - skipping expert merging")
